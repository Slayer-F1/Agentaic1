...
Every procedure is a versioned, approved `SKILL.md` in this repo (fields, eligibility rules, allowed services, approval chain, escalation) + a `PROFILE.md` persona. Those are seeded into n8n's own **Data Tables**, and the agent loads the matching skill **live from the `Skills` table** per request — no network call at runtime — becomes that procedure's expert, and every data/tool call passes a **service gateway** that enforces the skill's allowlist and writes an audit row — including denials. Adding a new procedure = one row in the `Skills` table (or, to author it in git: a skill `.md`, its entry in `registry/skills-index.json` — that index is what the build reads, not the folder — then rebuild + `deploy.py`). Zero n8n changes.
...
| `docker/` | One-command local stack: n8n + the portal (`docker compose -f docker/docker-compose.yml up -d`) |
...
| `workflow-split/` | The same system as 10 single-purpose workflows — easier to walk through node by node during the judges’ inspection. Import **one** set or the other, never both (identical webhook paths). `python tools/deploy.py --split` |
...
| `data/seeds/` | Synthetic registry seed (xlsx + CSVs, 10 tabs) — all fictional, labeled «تجريبي» |
...
Runtime tool choice (model picks services; gateway enforces the allowlist) · plan-first reasoning (think tool) · memory (conversation + cross-run registry) · automatic triggers (webhook + 12-hourly SLA sweep + daily reflection pass) · model-output branching (router, state machine, chaser decisions) · self-correction (deterministic working-day cross-check with forced re-reasoning, governance-denial recovery, retry+error workflow, needs_attention escalation).
